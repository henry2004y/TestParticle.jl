@testset "Output saving flags" begin
   ...
   sol = TP.solve(prob; dt = dt, savestepinterval = 10,
   ...
   sol = TP.solve(prob; dt = dt, savestepinterval = 10,
      save_everystep = false, save_start = true, save_end = true)[1]
   @test length(sol) == 2
   @test sol.t[1] == tspan[1]
   @test sol.t[end] == tspan[2]
   ...
   sol = TP.solve(prob; dt = dt, savestepinterval = 10,
      save_everystep = false, save_start = true, save_end = false)[1]
   @test length(sol) == 1
   @test sol.t[1] == tspan[1]

   # Scenario 5: Every step but no start/end
   sol = TP.solve(prob; dt = dt, savestepinterval = 10, save_everystep = true,
      save_start = false, save_end = false)[1]
   # Steps: 10, 20, ..., 990.
   @test length(sol) == 99
   ...
   sol = TP.solve(prob; dt = dt, savestepinterval = 3,
      save_everystep = true, save_start = true, save_end = true)[1]
   @test length(sol) == 335
   @test sol.t[1] == tspan[1]
   ...
   sol_ms = TP.solve(prob; dt = dt, savestepinterval = 10, n = 2,
      save_everystep = false, save_start = true, save_end = true)[1]
