...
function get_tokamak_topology()
    ...
    return tor_mesh = GeometryBasics.Mesh(points, faces)
end

# Passing proton in a Tokamak

## initial velocity for passing particle
v₀ = [0.0, 2.15, 3.1] .* 1.0e6
...
sol = solve(prob, Vern7(); dt = 2.0e-11);
...
prob = ODEProblem(trace!, stateinit, tspan, param)
sol = solve(prob, Vern7(); dt = 1.0e-11)
