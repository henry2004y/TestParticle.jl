...
prob_template = ODEProblem(trace, vcat(SA[0.0, 0.0, 0.0], SA[0.0, 0.0, 0.0]), tspan_cloud, param_cloud);

ensemble_prob_cloud = EnsembleProblem(prob_template; prob_func = prob_func_cloud)
sols_cloud = solve(ensemble_prob_cloud, Tsit5(), EnsembleThreads(); trajectories = N_cloud, seed);
