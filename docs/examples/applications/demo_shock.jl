...
n₁ = 1.0e6
T₁ = 720471.8506664868
Pi₁ = 0.0049735919716217296 * 1.0e-9
Pe₁ = 0.0049735919716217296 * 1.0e-9
Pth₁ = Pi₁ + Pe₁
V₁ = [-545.1484121835928, 0.0, 0.0] .* 1.0e3
B₁ = [0.0, 0.0, 5.0] .* 1.0e-9

n₂ = 3.1662479035540008e6
T₂ = 5.957947703036288e6
Pi₂ = 0.13022511153415584 * 1.0e-9
Pe₂ = 0.13022511153415584 * 1.0e-9
Pth₂ = Pi₂ + Pe₂
V₂ = [-172.17489874108816, 0.0, 0.0] .* 1.0e3
B₂ = [0.0, 0.0, 15.831239517770003] .* 1.0e-9;
...
function plot_dist(x, sols; nxchunks::Int = 2, ntchunks::Int = 20)
    ...
    for i in 1:nxchunks
        hist!(
            ax, vx[i], normalization = :pdf, bins = 50,
    ...
    text!(
        Point2f.(xmid .+ 400, 300.0), text = means_str, align = (:right, :center),
        offset = (-60, 0), color = :black, fontsize = 24
    )
    text!(
        Point2f.(xmid .+ 400, -700.0), text = std_str, align = (:right, :center),
        offset = (-60, 0), color = :black, fontsize = 24
    )
...
println("Vth₁ = ", round(vdf₁.vth / 1.0e3, digits = 2), " km/s") #hide
println("Vth₂ = ", round(vdf₂.vth / 1.0e3, digits = 2), " km/s") #hide
...
function collect_VDF(x, sols; ntchunks::Int = 20)
    ...
    for sol in sols
        for t in trange
            ...
            for i in 1:nxchunks
                if xrange[i] < xv[1] ≤ xrange[i + 1]
                    push!(table[i].vx, xv[4] / 1.0e3)
                    push!(table[i].vy, xv[5] / 1.0e3)
                    push!(table[i].vz, xv[6] / 1.0e3)
                end
            end
        end
    end
...
function plot_dist_pairplots(x, sols; ntchunks::Int = 20)
    ...
    l1 = @sprintf "x: [%d, %d] km downstream" xrange[1] / 1.0e3 xrange[2] / 1.0e3
    l2 = @sprintf "x: [%d, %d] km upstream" xrange[2] / 1.0e3 xrange[3] / 1.0e3
...
Pth₁ = Pi₁ + Pe₁
V₁ = [-545.1484121835928, 0.0, 0.0] .* 1.0e3
B₁ = [5.0, 0.0, 0.0] .* 1.0e-9

n₂ = 3.6363636363636362e6
T₂ = 7.380333520264822e6
Pi₂ = 0.18526630094290936 * 1.0e-9
Pe₂ = 0.18526630094290936 * 1.0e-9
Pth₂ = Pi₂ + Pe₂
V₂ = [-149.91581335048804, 0.0, 0.0] .* 1.0e3
B₂ = [5.0, 0.0, 0.0] .* 1.0e-9;
