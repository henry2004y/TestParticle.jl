...
function getinterp(A, gridx, gridy, gridz, order::Int=1)
   ...
   if order == 1
      itpx = interpolate(Ax, BSpline(Linear()))
      itpy = interpolate(Ay, BSpline(Linear()))
      itpz = interpolate(Az, BSpline(Linear()))
   elseif order == 2
      itpx = interpolate(Ax, BSpline(Quadratic()))
      itpy = interpolate(Ay, BSpline(Quadratic()))
      itpz = interpolate(Az, BSpline(Quadratic()))
   elseif order == 3
      itpx = interpolate(Ax, BSpline(Cubic(Line(OnGrid()))))
      itpy = interpolate(Ay, BSpline(Cubic(Line(OnGrid()))))
      itpz = interpolate(Az, BSpline(Cubic(Line(OnGrid()))))
   end
