...
title: Single tracing with additional diagnostics id: demo_savingcallback date: 2023-12-17 author: &quot;[Hongyang Zhou](https://github.com/henry2004y)&quot; julia: 1.9.4 description: Tracing one charged particle with additional diagnostics –-
...
The `SavingCallback` from DiffEqCallbacks.jl can be used to save additional outputs for diagnosis. Here we save the magnetic field along the trajectory, together with the parallel velocity. Note that `SavingCallback` is currently not compatible with ensemble problems; for multiple particle tracing with customized outputs, see [Demo: ensemble tracing with extra saving](@ref%20demo_output_func).
