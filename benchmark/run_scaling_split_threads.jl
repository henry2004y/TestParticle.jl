# SplitThreads Strong Scaling Driver
#
# Measures strong scaling of EnsembleSplitThreads by varying the number of
# worker processes while keeping the number of threads per worker constant.
#
# Usage:
#   julia --project benchmark/run_scaling_split_threads.jl
...
println("Starting split-threads scaling benchmark on worker counts: ", proc_counts)
...
ax = Axis(
    ...
    title = "EnsembleSplitThreads Scaling ($N_PARTICLES Particles)",
...
plot_path = joinpath(@__DIR__, "split_threads_scaling.png")
save(plot_path, fig)
println("Saved scaling plot to: ", plot_path)

# Save results to CSV
using DelimitedFiles
writedlm(joinpath(@__DIR__, "split_threads_scaling.csv"), [proc_counts times], ',')
println("Saved scaling results to: ", joinpath(@__DIR__, "split_threads_scaling.csv"))
